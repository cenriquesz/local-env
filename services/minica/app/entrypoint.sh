...
# Sincronizamos los certificados con el volumen named compartido
# (local-env-storage), para que proyectos externos con su propio
# compose.yaml puedan acceder a ellos declarandolo como
# volumen externo, sin depender de la ruta del host del bind mount.
SHARED_STORAGE_DIR="/local-env-storage"
if [ -d "$SHARED_STORAGE_DIR" ]; then
  mkdir -p "$SHARED_STORAGE_DIR/certs"
  cp -a $CERTS_DIR/. "$SHARED_STORAGE_DIR/certs/"
  chown -R 1000:1000 "$SHARED_STORAGE_DIR/certs"
fi
